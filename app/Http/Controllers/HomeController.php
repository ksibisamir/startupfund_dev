...
class HomeController extends Controller
{
    ...
    public function index(){
        ...
        $new_campaigns = Campaign::active()->where('for_purchase_funds',0)->orderBy('id', 'desc')->paginate(20);
        $funded_campaigns = Campaign::active()->where('for_purchase_funds',0)->funded()->orderBy('id', 'desc')->take(8)->get();
    ...
    public function newCampaignsAjax(){
        $new_campaigns = Campaign::whereStatus(1)->where('for_purchase_funds',0)->orderBy('id', 'desc')->paginate(20);
        $new_campaigns->withPath('ajax/new-campaigns');
    ...
    public function cominSoonPost(Request $request){
        $rules = [
            'email'  => 'required|email|unique:coming_soons',
        ];
        ...
        $create = ComingSoon::create($data);
        if ($create) {
            return back()->with('success', trans('app.coming_soon_saved_email'));;
        }
