<div class="single-donate-wrap">
    ...
    @if($campaign->user)
        <div class="single-author-box">
            ...
            <p>{{$campaign->user->startup_name}} <br/> <span>{{$campaign->address}}</span> </p>
        </div>
    @endif
    ...
    <div class="campaign-progress-info">
        <h4><strong>{!! get_amount($campaign->total_raised()) !!}</strong>
            @lang('app.raised_of') {!! get_amount($campaign->goal) !!} @lang('app.goal')
        </h4>
        ...
        <div class="progress">
            ...
            @endphp
            <div class="progress-bar" role="progressbar" aria-valuenow="{{$percent_raised}}" aria-valuemin="0"
                 aria-valuemax="100" style="width: {{$percent_raised <= 100 ? $percent_raised : 100}}%;">
                {{$percent_raised}}%
            </div>
        </div>

        <ul>
            <li>{{$campaign->days_left()}} @lang('app.days_left')</li>
            <li></li>
        </ul>
    </div>

    <div class="input-group">
        <input type="text" id="campaign_url_input" class="form-control"
               value="{{route('campaign_single', [$campaign->id, $campaign->slug])}}">
    ...
    <div class="donate_form">
        @if(!Auth::user() or (Auth::user() and Auth::user()->investor_contract_accepted) )
        <h2>@lang('app.donate')</h2>

            @if( ! $is_ended and !Auth::user() or (Auth::user() and Auth::user()->investor_contract_accepted))

                {{Form::open([ 'route' => 'add_to_cart', 'class' => 'form-horizontal'])}}
                <input type="hidden" name="campaign_id" value="{{$campaign->id}}"/>
                <div class="input-group donate_amount_field {{ $errors->has('amount')? 'has-error':'' }} " >
                    <input type="number" step="0.001" lang="en" min="100" name="amount" id="amount" class="form-control"
                           value="{!! get_amount_raw($campaign->recommended_amount) !!}"
                           placeholder="@lang('app.enter_amount')"/>
                    <div class="input-group-addon">{!! get_currency_symbol(get_option('currency_sign')) !!}</div>
                </div>
                    <div style="clear: both">
                        <p class="help-block has-error subscription-amount-error">
...
{{--                @endif--}}
                ...
                <div class="donate-form-button action-group">
