...
@section('content')
    @if( ! empty($title))
        <div>
            <h1 class="page-header"> {{ $title }}
                ...
                <a href="{{route('withdrawal_preference')}}" class="btn btn-primary pull-right"><i
                            class="fa fa-money"></i> @lang('app.withdrawal_preference')</a>
            </h1>
        </div> <!-- /.row -->
    @endif
    ...
    <table class="table table-bordered table-striped">
        ...
        @foreach($campaigns as $campaign)
            <tr>
                <td><a href="{{route('campaign_single', [$campaign->id, $campaign->slug])}}"
                       target="_blank"> {{$campaign->title}} </a></td>
                <td>{!! get_amount($campaign->amount_raised()->amount_raised) !!}</td>
                <td>{!! get_amount($campaign->amount_raised()->campaign_owner_commission) !!}
                    ({{$campaign->campaign_owner_commission}}%)
                </td>
                <td>
                    @if($campaign->is_ended())
                        ...
                        @else
                            <a href="javascript:;" class="btn btn-info btn-xs" data-toggle="modal"
                               data-target="#{{$campaign->id}}-withdraw-modal">
                                @lang('app.withdraw')
                            </a>
                        @endif
                    @else
        ...
        <table class="table table-bordered table-striped">
            ...
            @foreach($withdrawal_requests as $withdraw)
                <tr>
                    ...
                    <td><a href="{{route('withdraw_request_view', $withdraw->id)}}" class="btn-link"><i
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">@lang('app.your_withdraw_amount')
                            <strong>{!! get_amount($campaign->amount_raised()->campaign_owner_commission) !!}</strong>
                        </h4>
                    </div>
                    <div class="modal-body">

                        <h4>@lang('app.default_withdrawal_account'): <strong>{{withdrawal_preference()}}</strong> <a
                                    href="{{route('withdrawal_preference')}}"
                                    class="btn xs btn-link">@lang('app.update') </a></h4>
