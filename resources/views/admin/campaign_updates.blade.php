...
@section('content')
    @if( ! empty($title))
        <div>
            <h1 class="page-header"> {{ $title }} <a href="{{route('edit_campaign', $campaign_id)}}"
                ...
                <table class="table table-bordered categories-lists">
                    ...
                    @foreach($updates as $update)
                        <tr>
                            ...
                            <td width="100">
                                <a href="{{ route('update_update', [$update->campaign_id,$update->id]) }}"
                                   class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> </a>
                                <a href="javascript:;" class="btn btn-danger btn-xs" data-id="{{ $update->id }}"><i
