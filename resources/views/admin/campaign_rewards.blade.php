...
@section('content')
    ...
    @if( ! empty($title))
        <div>
            <h1 class="page-header"> {{ $title }} <a href="{{route('edit_campaign', $campaign->id)}}"
                                                     class="btn btn-info pull-right"><i
                            class="fa fa-arrow-circle-o-left"></i> @lang('app.back_to_campaign')</a></h1>
        </div> <!-- /.row -->
    @endif
    ...
    <div class="row">
        <div class="col-sm-8 col-sm-offset-1 col-xs-12">
            ...
            <div class="form-group {{ $errors->has('amount')? 'has-error':'' }}">
                ...
                <div class="col-sm-8">
                    <input type="number" class="form-control" id="amount" value="{{ old('amount') }}" name="amount"
            ...
            <div class="form-group {{ $errors->has('estimated_delivery')? 'has-error':'' }}">
                <label for="estimated_delivery"
                       class="col-sm-4 control-label">@lang('app.estimated_delivery')</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" id="estimated_delivery"
                           value="{{ old('estimated_delivery') }}" name="estimated_delivery"
                           placeholder="@lang('app.estimated_delivery')">
                ...
                <table class="table table-bordered categories-lists">
                    ...
                    @foreach($rewards as $reward)
                        <tr>
                            ...
                            <td width="100">
                                <a href="{{ route('reward_update', [$reward->campaign_id,$reward->id]) }}"
                                   class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> </a>
                                <a href="javascript:;" class="btn btn-danger btn-xs" data-id="{{ $reward->id }}"><i
