            ...
            @if($users->count() > 0)
                ...
                <table class="table table-bordered table-striped">
                    <tr>
                        ...
                        <td>@lang('app.kyc')</td>
                        <td>@lang('app.unique_identifier')</td>
                    ...
                    @foreach($users as $user)
                        <tr>
                            <td>
                                <img src="{{ $user->get_gravatar(30) }}"
                                     class="img-thumbnail img-circle" width="30"/>
                                {{ $user->name }}
                            </td>
                            <td>{{$user->email}}</td>
                            <td>
                            ...
                            <td>
                                {{$user->tledger_wallet_id }}
                            </td>
                            <td>
                                <a href="{{route('users_view', $user->id)}}"
                                ...
                                @if($user->active_status == 0)
                                    <a href="{{route('user_status', [$user->id, 'approve'])}}"
                                       ...
                                       title="@lang('app.approve')"><i class="fa fa-ban"></i> </a>

                                    <a href="{{route('user_status', [$user->id, 'block'])}}"
                                       class="btn btn-danger btn-sm" data-toggle="tooltip"
                                       title="@lang('app.block')"><i class="fa fa-ban"></i> </a>
                                ...
                                @elseif($user->active_status == 2)
                                    <a href="{{route('user_status', [$user->id, 'approve'])}}"
                                       ...
                                       title="@lang('app.approve')"><i class="fa fa-check-circle-o"></i>
                                    </a>
                                @endif

                                <a href="{{route('users_edit_step_1', $user->id)}}"
